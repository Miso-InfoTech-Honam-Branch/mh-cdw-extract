"""고급 추출 파이프라인의 소스 해석, 자동 PIVOT, 검증과 미리보기를 조정한다."""
...
def _resolve_automatic_pivot_values(
    ...
    for index, step in enumerate(steps):
        if (
            ...
            or str(step.get("type") or "").upper() != "PIVOT"
        ):
            continue
        ...
        prefix = {
            **resolved,
            "steps": [
                *steps[:index],
                {
                    "stepId": "__pivot_discovery_output",
                    "type": "OUTPUT",
                    "enabled": True,
                    "config": {},
                },
        ...
        compiled_prefix = compile_pipeline(
            f"SELECT * FROM {source_sql}", source_schema, prefix
        )
        pivot_column = next(
            (
                item
                for item in compiled_prefix.output_schema
                if item.column_id == pivot_column_id
            ),
            None,
        )
        ...
        for mapping in code_mappings or []:
            source_matches = any(
                (column.get("alias") or column.get("name"))
                == pivot_column.physical_name
                and column.get("name") == mapping.get("sourceColumn")
                and (column.get("table") or None)
                == (mapping.get("sourceTable") or None)
...
def compile_pipeline_request(
    connection_id: str, request: dict, data_root: str | Path, connection
):
    """요청 소스와 자동 PIVOT 값을 해석해 재현 가능한 파이프라인을 컴파일한다."""
    ...
    # 자동 PIVOT 값도 컴파일 스냅샷 안에 고정한다. 이후 실행과 재시도는
    # 이 해시를 비교해 값 발견 시점이 다른 파이프라인을 섞지 않는다.
    ...
    if expected_pipeline and expected_pipeline != compiled.pipeline_hash:
        raise PipelineSnapshotMismatch(
            "PIPELINE_SNAPSHOT_MISMATCH: 저장된 변환 단계가 검증본과 다릅니다."
        )
    return compiled


def validate_pipeline_request(
    connection_id: str, request: dict, data_root: str | Path
) -> dict:
    """파이프라인을 실행하지 않고 소스 스키마와 단계별 출력 계약을 검증한다."""
...
def preview_pipeline(connection_id: str, request: dict, data_root: str | Path) -> dict:
    """컴파일된 파이프라인을 제한된 행 수로 실행해 미리보기를 반환한다."""
    ...
    try:
        compiled = compile_pipeline_request(connection_id, request, data_root, conn)
        result = conn.execute(
            f"SELECT * FROM ({compiled.sql}) AS __preview LIMIT {limit}",
            compiled.parameters,
        )
        ...
        return {
            **compiled.json(False),
            ...
            "limit": limit,
            "rowCount": len(rows),
            "columns": [item.json() for item in compiled.output_schema],
